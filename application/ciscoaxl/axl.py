...
class axl(object):
    ...
    def get_directory_numbers(
        ...
        """
        try:
            return self.client.listLine(searchCriteria={"pattern": "%"}, returnedTags=tagfilter,)[
                "return"
            ]["line"]
        except Fault as e:
            return e
    ...
    def Add_line(self, **args):
        ...
        """
        try:
            return self.client.addLine(**args)
        except Fault as e:
            return e
    ...
    def add_directory_number(
        ...
        forward_to_vm_noReply="false",
        forward_to_vm_busy="false",
    ):
        """
            ...
            return self.client.addLine(
                {
                    ...
                    "callForwardBusy": {
                        "forwardToVoiceMail": forward_to_vm_busy,
                        "callingSearchSpaceName": call_forward_css,
                    ...
                    "callForwardNoAnswer": {
                        "forwardToVoiceMail": forward_to_vm_noReply,
                        "callingSearchSpaceName": call_forward_css,
                        "destination": call_forward_destination,
                    },
    ...
    def get_phones(self, name, tagfilter={
            "name": "",
            "product": "",
            "description": "",
            "protocol": "",
        ...
        def inner(skip):
            while True:
                res = self.client.listPhone(
                            searchCriteria={"name": f"%{name}%"}, returnedTags=tagfilter, first=1000, skip=skip
                        )["return"]
                skip=skip+1000
    ...
    def get_device_profiles(
        ...
        try:
            return self.client.listDeviceProfile(
                {"name": f"%{name}%"}, returnedTags=tagfilter,
            )
        except Fault as e:
            return e
    ...
    def add_device_profile(
        ...
        description="",
        product="Cisco 8811",
        phone_template="Standard 8811 SIP",
        ...
        if "return" in resp and resp["return"] is not None:
            ...
            try:
                return self.client.updateUser(
                    userid=user_id,
                    ctiControlledDeviceProfiles={"profileName": {"uuid": uuid}},
                    phoneProfiles={"profileName": {"uuid": uuid}},
                    defaultProfile=default_profile,
                    subscribeCallingSearchSpaceName=subscribe_css,
                    primaryExtension={"pattern": primary_extension[0],"routePartitionName":primary_extension[1]},
                    associatedGroups={"userGroup": [{"name": "Standard CCM End Users"},{"name":"Standard CTI Enabled"}]},
                )
            except Fault as e:
                return e
        else:
    ...
    def get_route_plan(
        ...
        """
        try:
            return self.client.listRoutePlan({"dnOrPattern":"%"}, returnedTags=tagfilter,)
        except Fault as e:
            return e
