...
const dispatch= useDispatch()
  ...
  const  checkUser= async(input)=>{
    ...
    const data={"UserId":String(input.target.value)}
     const checkUser= await axiosClient.post("/checkUserState",data)
    .then((res) => {
     if( res.data.data){
...
// }
  ...
  const onSubmit = async (mo) => {
    ...
    console.log(mo)
    setLoading(true)
    const response=await axiosClient.post("/AddUser",mo)
      .then((Res) => {
 ...
 const CheckPartition=async()=>{
  // setPLoad(true)
  const response=await axiosClient.get("/CheckPartition").then(res=>{
